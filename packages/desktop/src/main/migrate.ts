...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.necode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `necode.settings.dat` is special: it maps to the `necode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"necode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  const storeName = filename === "necode.settings.dat" ? "necode.settings" : filename
