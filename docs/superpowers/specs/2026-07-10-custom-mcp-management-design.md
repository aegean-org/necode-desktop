...
MCP 设置页标题区增加“添加 MCP”操作。标题保持左对齐，“添加 MCP”按钮位于同一标题行最右侧。列表先显示产品内置项，再按展示名称排列用户自定义项。
...
- “粘贴 MCP 配置”：接受单个 MCP 条目或包含 `mcp` 包装层的 JSON/JSONC 配置，解析后进入可检查、可修改的表单；
- “手动配置”：直接进入完整高级表单。

导入结构按固定语法判定：当根对象的唯一 `mcp` 值包含字面量 `type: "local" | "remote"` 时，将其解释为名称为 `mcp` 的直接条目；否则将根 `mcp` 值解释为包装映射。包装映射中的条目仍可命名为 `type`。无效 JSONC、重复键、多个包装条目和未知字段都必须明确失败。

粘贴配置必须自动识别名称、本地命令参数、工作目录、环境变量、远程 URL、Header、OAuth、超时和启用状态。解析失败、包含多个条目或结构不受支持时显示明确错误，不得猜测条目、静默丢弃字段或直接写盘。用户确认表单并保存前不产生持久化修改。

编辑入口直接打开完整表单，不显示添加方式选择。

对话框复用设置 v2 的控件，并提供“本地命令”和“远程 URL”两种类型。标题和底部操作栏固定，中间字段区域独立滚动，确保任意受支持窗口高度下都能访问全部字段和保存操作。
...
- 第一行固定表示可执行命令且不显示删除按钮；后续参数行使用明确的移除图标；“添加参数”使用独立的文字按钮；
- 可选工作目录；
- 可重复添加的环境变量键值行。
...
- 未注册的设置 v2 图标不得静默显示成其他图标；命令参数和键值行的移除操作必须使用已注册且语义明确的图标。
- 粘贴配置解析覆盖单条配置、`mcp` 包装配置、JSONC 注释、无效 JSONC、多个条目和不受支持字段。

### 验证

- 分别从 `packages/opencode` 和 `packages/app` 运行聚焦 Bun 测试，后端测试命令限制在 60 秒内。
- 从 `packages/app` 运行 `bun run test:browser` 验证浏览器条件下的真实对话框交互测试。
...
- 公共 MCP 目录或一键市场安装。
- 配置导出、配置包批量导入或批量编辑；仅支持粘贴并检查一个 MCP 条目。
- 超出现有配置文件模型的密钥管理器集成。
- 在一次写操作中跨作用域移动条目。
